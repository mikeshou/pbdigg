...
var arrFontsize=[{n:'xx-small',wkn:'x-small',s:'8pt',t:'极小'},{n:'x-small',wkn:'small',s:'10pt',t:'特小'},{n:'small',wkn:'medium',s:'12pt',t:'小'},{n:'medium',wkn:'large',s:'14pt',t:'中'},{n:'large',wkn:'x-large',s:'18pt',t:'大'},{n:'x-large',wkn:'xx-large',s:'24pt',t:'特大'}];
...
var htmlLink='<div>链接地址: <br /><input type="text" id="xhEdtLinkHref" value="http://" class="text" /></div><div style="text-align:center;"><input type="button" id="xhEdtSave" value="确定" />&nbsp;<input type="button" id="xhEdtCancel" value="取消" /></div>';
var htmlImg='<div>图片地址：<input type="text" id="xhEdtImgSrc" value="http://" class="text" /></div><div>替换文本：<input type="text" id="xhEdtImgAlt" /></div><div>宽度：<input type="text" id="xhEdtImgWidth" style="width:40px;" /> 高度：<input type="text" id="xhEdtImgHeight" style="width:40px;" /></div><div style="text-align:center;"><input type="button" id="xhEdtSave" value="确定" />&nbsp;<input type="button" id="xhEdtCancel" value="取消" /></div>';
var htmlFlash='<div>动画地址：<input type="text" id="xhEdtFlashSrc" value="http://" class="text" /></div><div>宽度：<input type="text" id="xhEdtFlashWidth" style="width:40px;" value="480" /> 高度：<input type="text" id="xhEdtFlashHeight" style="width:40px;" value="400" /></div><div style="text-align:right;"><input type="button" id="xhEdtSave" value="确定" /></div>';
var htmlMedia='<div>视频地址：<input type="text" id="xhEdtMediaSrc" value="http://" class="text" /></div><div>宽度：<input type="text" id="xhEdtMediaWidth" style="width:40px;" value="480" /> 高度：<input type="text" id="xhEdtMediaHeight" style="width:40px;" value="400" /></div><div style="text-align:right;"><input type="button" id="xhEdtSave" value="确定" /></div>';
...
var toolsThemes={
	Basic:'GStart,Bold,Italic,Underline,Strikethrough,GEnd,Separator,GStart,FontColor,BackColor,GEnd,Separator,GStart,Link,Unlink,GEnd,Separator,GStart,Source,GEnd',
	PBDigg:'GStart,Blocktag,Fontface,FontSize,GEnd,Separator,GStart,Cut,Copy,Paste,Pastetext,GEnd,Separator,GStart,Bold,Italic,Underline,Strikethrough,FontColor,BackColor,Removeformat,GEnd,Separator,GStart,Align,List,Outdent,Indent,GEnd,Separator,GStart,Link,Unlink,Img,Flash,Media,Emot,Table,GEnd,Separator,GStart,Source,GEnd'};
$.xheditor=function(textarea,options)
{
	var defaults={skin:'pbdigg',tools:'PBDigg',internalScript:false,inlineScript:false,internalStyle:false,inlineStyle:true,showBlocktag:false,forcePtag:true,keepValue:true,upLinkExt:"zip,rar,txt",upImgExt:"jpg,jpeg,gif,png",upFlashExt:"swf",upMediaExt:"avi",modalWidth:350,modalHeight:220,modalTitle:true};
	...
	if(_this.settings.tools.match(/^\s*(Basic|PBDigg)\s*$/i))
	{
	...
	this.init=function()
	{
		...
		if(_jPanel.size()==0)
		{
			...
			_jCntLine=$('<div id="xhEdtCntLine"><img src="'+jsURL+'skin/'+_this.settings.skin+'/img/spacer.gif" /></div>');
		...
		xCount++;
		bInit=true;
		if(_this.settings.sourceMode)setTimeout(_this.toggleSource,20);
		return true;
	}
	...
	{
		var jLink=$(htmlLink);
		var jParent=_this.getParent('a'),jHref=$('#xhEdtLinkHref',jLink),jSave=$('#xhEdtSave',jLink),jCancel=$('#xhEdtCancel',jLink);
		if(jParent.size()==1)
		{
		...
		jSave.click(function(){
			_this.focus();
			var url=jHref.val();
			if(url=='')_this._exec('unlink');
			if(url!='')
			{
				if(jParent.size()==0)
				{
				//	var selection = _doc.getSelection();
				//	var inserText = selection ? selection : url;
				//	_this.pasteHTML('<a href="#xhedt_tempurl">'+inserText+'</>',true);
					_this._exec('createlink','#xhedt_tempurl');
					jParent=$('a[href$="#xhedt_tempurl"]',_doc);
				}
				_this.attr(jParent,'href',url);
				jParent.attr('target','_blank');
			}
			_this.hidePanel();
			return false;
		});
	...
	{
		var jImg=$(htmlImg);
		var jParent=_this.getParent('img'),jSrc=$('#xhEdtImgSrc',jImg),jAlt=$('#xhEdtImgAlt',jImg),jWidth=$('#xhEdtImgWidth',jImg),jHeight=$('#xhEdtImgHeight',jImg),jSave=$('#xhEdtSave',jImg),jCancel=$('#xhEdtCancel',jImg);
		if(jParent.size()==1)
		{
	...
	{
		var jEmot=$('<div class="xhEdtEmot"></div>'),jLine,jItem,c=0,sEmotPath=pb_url+'images/emoticons/';
		jLine=$('<div></div>');
