...
class Watermark
{
	...
	*/
	function Watermark()
	{
		global $pb_watertext, $pb_waterfont, $pb_waterfontsize, $pb_waterfontcolor, $pb_waterimg, $pb_watertransition, $pb_waterquality, $pb_waterposition;

		$this->_text = $pb_watertext ? $pb_watertext : 'PBDigg.Net';
		$this->_tfont = strtolower(trim(PBDIGG_ROOT.'images/font/'.$pb_waterfont));
		$this->_tsize = (int)$pb_waterfontsize;
		$this->_tcolor = $pb_waterfontcolor;
		$this->_wimg = PBDIGG_ROOT.'images/water/'.$pb_waterimg;
		$this->_transition = $pb_watertransition;
		$this->_jquality = $pb_waterquality;
		$this->_pos = $pb_waterposition;
	}
	/**
	 * 设置图片路径
	 */
	function setImg($var)
	{
		global $pb_waterminsize, $pb_watertype;
		...
		list($minh, $minw) = explode("\t", $pb_waterminsize);
	...
	*/
 	function getPosition($type = 'image')
	{
  	...
  	function gb2utf8($gb)
  	{
    	...
    	$filename = PBDIGG_ROOT.'include/encoding/gb-unicode.table';
    	$tmp=file($filename);
    	$codetable=array();
